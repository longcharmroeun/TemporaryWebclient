...
namespace CloudflareSolverRe.Types.Javascript
{
    ...
    {
        private static readonly Regex JsChallengeRegex = new Regex(@"<script.*?>(?<script>.*?var s,t,o,p, b,r,e,a,k,i,n,g,cpo,f, (?<className>\w+?)={""(?<propName>\w+?)"":(?<propValue>.*?)};.*?k = '(?<k>.*?)';.*?(?<calculations>\S*?\w+?\.\w+?[+\-*\/]=(?:(?<normal>(?:\+|\-|\(|\)|\!|\[|\]|\/)+?;)|(?<charCode>(?:\+|\-|\(|\)|\!|\[|\]|\/)+?\(function.*?}\(.*?\)\)\);)|(?<cfdn>function\(.\)\{var.*?;\s.*?;)))+.*?a\.value\s=\s\(\+\w+\.\w+(\s\+\s(?<addHostLength>t\.length))*?\)\.toFixed\((?<round>\d+)\);.*?},\s*(?<delay>\d+)\);.*?)<\/script>.*?<form.+?action=""(?<action>\S+?)"".*?>.*?name=""r"" value=""(?<r>\S+)"".*?value=""(?<jschl_vc>[a-z0-9]{32})"".*?id=""jschl-vc"".*?name=""pass"" value=""(?<pass>\S+?)"".*?</form>.*?(style="".*?"">(?<cf_dn>.*?)</div>\s+<div.*?){0,1}\s+</div>", RegexOptions.Singleline/* | RegexOptions.Compiled*/);
